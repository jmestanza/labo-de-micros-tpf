...
char *buffer_ptr = buffer; // Porque no puedo comparar directamente con string sino

uint8_t *data2send = "Temperature"; // Mensaje de prueba
...
void bt_callback()
{
    ...
    if(((kUART_TxDataRegEmptyFlag) & UART_GetStatusFlags(UART_2_PERIPHERAL)) && (txOnGoing))
    {
    	...
    	if(cont == 11) // Esto deberia ser el size de los datos
    	{
