wave = [0x2F, 0x00, 0x2F, 0x00, 0x93, 0x08, 0x93, 0x08, 0x1D, 0x14, 0x1D, 0x14, 0xA3, 0x1E, 0xA3, 0x1E, 0x77, 0x29,
...
print(hex((wave[0]<<8)+wave[1])) # 2f00
print(hex((wave[1]<<8)+wave[0])) # 002f

first_byte_arr = wave[::2]
second_byte_arr = wave[1::2]

wave_msb_first = [(first_byte_arr[i]<<8) + second_byte_arr[i] for i in range(len(first_byte_arr))]
wave_lsb_first = [(second_byte_arr[i]<<8) + first_byte_arr[i] for i in range(len(first_byte_arr))]


msb_right_channel = wave_msb_first[::2] ## saltea de a 2
msb_left_channel = wave_msb_first[1::2] ## saltea de a 2

lsb_right_channel = wave_lsb_first[::2] 
lsb_left_channel = wave_lsb_first[1::2] ## saltea de a 2

import matplotlib.pyplot as plt
import numpy as np
print([hex(val) for val in msb_left_channel][:10])

# plt.plot(msb_left_channel)
# plt.show()

# plt.plot(lsb_left_channel)
# plt.show()

threshold = (2**16/2)
print(threshold)

# parece que es lsb pero hay saltos porque es todo positivo, pienso que hay que ajustar la señal

adjusted_lsb_left_channel = np.array([el-(2**16-1) if el >= threshold  else el for el in lsb_left_channel])

fs = 16000
time = np.arange(0,len(adjusted_lsb_left_channel)/fs,1/fs)
plt.plot(time,adjusted_lsb_left_channel)
plt.show()

print(1/(0.16933-0.16476)) # 200Hz aprox la señal
print("la señal dura ",len(adjusted_lsb_left_channel)/fs,"segundos")
...
data_left = np.array(lsb_left_channel).astype(np.int16)
data_right = np.array(lsb_right_channel).astype(np.int16)
...
write('music.wav', fs, data.T)

# print([hex(val) for val in second_byte_arr][:10])