...
void i2s_init(void){
	...
	i2s_ptr->MDR =  (i2s_ptr->MDR & ~I2S_MDR_DIVIDE_MASK) | I2S_MDR_DIVIDE(999); // 12 bits
...
//	tx_cfg.tx_cfg_2_reg.master_clock_select = master_clock_1;
	...
	tx_cfg.tx_cfg_2_reg.bit_clock_divide = 49; // divides by (DIV+1)*2 // 8 bits
	...
	tx_cfg.tx_cfg_4_reg.sync_width = 15; // ?? // #bit_clks_deseados - 1
	// esto deberia ser la mitad, para que sea L-R-L-R
	// ahora hay una sola palabra por frame de 32 bits.
	// estoy transmitiendo 0xafaf afaf
	// =>1 bit clock por bit, entonces (32/2)-1  = 15 bit clocks
